...
namespace Modules.CodeGenerator.Template.Blazor.Pages.CodeTemplate
{
    partial class CodeTemplateTree
    {
        ...
        private async Task FetchPageDataAsync()
        {
            ...
            _ = _message.SuccessAsync($@"合计{TableModel.TotalCount}条数据。");
        }
        #region 增删改
        ...
        {
            this.IsAdd = true;
            this.EditRow = new CodeTemplateTreeVM();
            Dialog.Title = "添加数据";
            Dialog.Visible = true;
        }
        ...
        private async Task DelClick()
        {
            ...
            if (confirmResult == ConfirmResult.OK)
            {
                ...
                if (ret) _ = _message.SuccessAsync("删除成功"); else _ = _message.ErrorAsync("删除失败");
            }
        }
