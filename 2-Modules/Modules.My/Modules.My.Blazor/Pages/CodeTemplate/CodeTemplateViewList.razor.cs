    ...
    partial class CodeTemplateViewList
    {
        ...
        private async Task ViewDataModelInitAsync()
        {
            ...
            if (ret.Code == Core.Shared.ResultType.Success)
            {
                ...
                if (!ObjViewDataModel.IsValid()) _ = _message.ErrorAsync("取得视图信息错误。");
            ...
            {
                _ = _message.ErrorAsync($@"视图 {View_id} 初始化失败。" + ret.Msg);
            }
        }
        ...
        private async Task FetchPageDataAsync()
        {
            ...
            await InvokeAsync(StateHasChanged);
            _ = _message.SuccessAsync($@"当前第{ObjViewDataModel.DictData.PageIndex}页,合计{ObjViewDataModel.DictData.TotalCount}条数据。");
            ...
            _modalService.ConfirmAsync(new ConfirmOptions()
            {
                ...
                OnOk = async (e) =>
                {
                    ...
                    if (ret)
                    {
                        ...
                        await InvokeAsync(StateHasChanged); //强制刷新
                        _ = _message.SuccessAsync($@"删除{count}行数据成功");
                    }
                    else
                    {
                        _ = _message.ErrorAsync("删除失败。");
