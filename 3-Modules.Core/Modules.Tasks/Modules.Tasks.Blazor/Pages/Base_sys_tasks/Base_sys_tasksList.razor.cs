...
namespace Modules.Tasks.Blazor.Pages.Base_sys_tasks
{
    partial class Base_sys_tasksList
    {
        ...
        private TableDataModel<Base_sys_tasksVM> TableDataModel { get; set; } = new TableDataModel<Base_sys_tasksVM>(); //这里使用无参数构造函数，为了AntTable初始化
        ...
        {
            var confirmResult = await _confirmService.Show("确认停止此任务吗？", "", ConfirmButtons.OKCancel);
            if (confirmResult == ConfirmResult.OK)
            {
                ...
                if (ResuleModel.Code == Core.Shared.ResultType.Success)
                {
                    ...
                    if (ret)
                    {
                        _ = _message.Success("任务停止成功 。");
                    }
                    else
                    {
                        _ = _message.Error("存盘失败,");
                    }
                }
                else
                {
                    _ = _message.Error("任务停止失败：" + ResuleModel.Msg);
                }
            }
        }
        public async Task OnStart(Base_sys_tasksVM model)
        {
            if (model.Is_start)
            {
                _ = _message.Info("此任务已经启动，无法再次启动 。");
                return;
            }
            ...
            {
                _ = _message.Error("任务启动失败：" + String.Join(',', ResuleModel.Msg));
            }
        ...
        private void EditClick(Base_sys_tasksVM editRow)
        {
            ...
            Dialog.Title = "编辑数据:" + this.EditRow.Id;
            Dialog.Visible = true;
        }
        private void DelClick(Base_sys_tasksVM editRow)
        {
            if (editRow.Is_start)
            {
                _ = _message.Success("任务需要停止后，才能删除。");
                return;
            }
