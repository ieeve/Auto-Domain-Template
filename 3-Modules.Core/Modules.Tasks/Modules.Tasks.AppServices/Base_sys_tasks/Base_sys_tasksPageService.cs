...
{
    public class Base_sys_tasksPageService : IBase_sys_tasksPageService
    {
        ...
        public TableDataModel<Base_sys_tasksVM> TableDataModel { get; private set; }
        public Core.Domain.ObjectDomain.ObjectModel ObjectModel { get; private set; }
        public Base_sys_tasksPageService(IMapper _mapper, IAppConfigServices AppConfigService, IObjectService objectService, IBase_sys_tasksRepository Repository, IBase_logService logService, IBase_user_columnsService user_ColumnsService, ICurrentUserService CurrentUser)
        {
        ...
        public async Task InitAsync()
        {
            ...
            this.TableDataModel = new TableDataModel<Base_sys_tasksVM>(table_name, _repository.DbClient.CurrentConnectionConfig.DbType);
            ...
            else
            {
                ...
                this.TableDataModel.SetColumnHeaderModels(ColumnHeaderService.GetColumnHeaderModelFromVM<Base_sys_tasksEntity>(table_name));
            }
        ...
        public void SetTableModelDataSource(List<Base_sys_tasksVM> list)
        {
            if (this.TableDataModel == null) this.TableDataModel = new TableDataModel<Base_sys_tasksVM>();
        ...
        public async Task QueryPageDataAsync()
        {
            ...
            this.TableDataModel.TableModel.DataSource = _mapper.Map<List<Base_sys_tasksVM>>(data);
        }
        ...
        #region 添加，删除，修改
        public async Task<bool> RemoveRowDataAsync(Base_sys_tasksVM row)
        {
            //记录删除日志
            Base_sys_tasksEntity beforeData = null;
        ...
        public async Task<bool> RemoveRowDataAsync(List<Base_sys_tasksVM> rows)
        {
            var IDs = rows.Select(s => s.Id).ToArray();
            //记录删除日志
            List<Base_sys_tasksEntity> beforeData = null;
        ...
        public async Task<bool> AddRowDataAsync(Base_sys_tasksVM row)
        {
            row.Id = YitIdHelper.NextId().ToString();
            row.Createtime = DateTime.Now;
            row.Createuid = _currentUserService.CurrentUser.Authname;
            row.Tenant_id = _currentUserService.CurrentUser.Tenant_id;
            var ret = await _repository.InsertAsync(_mapper.Map<Base_sys_tasksEntity>(row));
        ...
        public async Task<bool> AddRowDataAsync(List<Base_sys_tasksVM> rows)
        {
            //如果没有主键，添加主键值
            NullIdServer<Base_sys_tasksVM>.ProcessNullId(rows);
            ...
            var ret = await _repository.InsertAsync(_mapper.Map<List<Base_sys_tasksEntity>>(rows));
            if (ret > 0 && this.TableDataModel != null)
            {
        ...
        public async Task<bool> UpdateRowDataAsync(Base_sys_tasksVM row)
        {
            ...
            bool ret = await _repository.UpdateAsync(_mapper.Map<Base_sys_tasksEntity>(row));
            if (ret && this.TableDataModel != null)
            {
        ...
        public async Task<bool> UpdateRowDataAsync(List<Base_sys_tasksVM> rows)
        {
            foreach (var row in rows)
            {
                row.Updatetime = DateTime.Now;
            ...
            bool ret = await _repository.UpdateAsync(_mapper.Map<List<Base_sys_tasksEntity>>(rows));
            if (ret && this.TableDataModel != null)
            {
                this.TableDataModel.TableModel.RemoveRow(rows);
